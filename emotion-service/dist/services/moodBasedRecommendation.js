...
exports.MoodBasedRecommendationService = void 0;
class MoodBasedRecommendationService {
    constructor() {
        this.comfortFoodCuisines = new Map();
        this.celebratoryDiningOptions = new Map();
        this.therapeuticCuisines = new Map();
     ...
     */
    generateMoodBasedRecommendations(request) {
     ...
     */
    identifyComfortFood(emotion, intensity, context) {
        ...
        if (context?.socialSetting === 'alone') {
            // Solo comfort dining
            recommendations = recommendations.filter(rec => !rec.specificDishes?.some(dish => dish.toLowerCase().includes('sharing')));
        }
     ...
     */
    suggestCelebratoryDining(emotion, intensity, context) {
     ...
     */
    handleNeutralState(userPreferences, context) {
        let recommendations = [
            {
                cuisineType: 'Chinese',
                matchScore: 0.8,
     ...
     */
    determineRecommendationType(request) {
     ...
     */
    generateComfortFoodRecommendations(request) {
     ...
     */
    generateCelebratoryRecommendations(request) {
     ...
     */
    generateTherapeuticRecommendations(request) {
     ...
     */
    generateAdventurousRecommendations(request) {
        return [
            {
                cuisineType: 'Fusion',
                matchScore: 0.9,
     ...
     */
    generateNeutralRecommendations(request) {
     ...
     */
    generateAtmosphereAdjustments(request, type) {
        const adjustments = [];
        switch (type) {
            case 'comfort':
                adjustments.push({ factor: 'cozy_atmosphere', adjustment: 'increase', weight: 0.9, reasoning: 'Cozy environment provides emotional comfort' }, { factor: 'quiet_atmosphere', adjustment: 'increase', weight: 0.7, reasoning: 'Quiet setting helps with emotional processing' }, { factor: 'warm_lighting', adjustment: 'increase', weight: 0.6, reasoning: 'Warm lighting creates comforting ambiance' });
                break;
            case 'celebratory':
                adjustments.push({ factor: 'festive_atmosphere', adjustment: 'increase', weight: 0.9, reasoning: 'Festive environment matches celebratory mood' }, { factor: 'social_atmosphere', adjustment: 'increase', weight: 0.8, reasoning: 'Social setting enhances celebration' }, { factor: 'upscale_atmosphere', adjustment: 'increase', weight: 0.7, reasoning: 'Special occasions warrant elevated dining' });
                break;
            case 'therapeutic':
                adjustments.push({ factor: 'calm_atmosphere', adjustment: 'increase', weight: 0.9, reasoning: 'Calm environment helps reduce stress' }, { factor: 'peaceful_atmosphere', adjustment: 'increase', weight: 0.8, reasoning: 'Peaceful setting promotes relaxation' }, { factor: 'minimal_atmosphere', adjustment: 'increase', weight: 0.6, reasoning: 'Minimal design reduces overstimulation' });
                break;
            case 'adventurous':
                adjustments.push({ factor: 'unique_atmosphere', adjustment: 'increase', weight: 0.9, reasoning: 'Unique setting matches adventurous spirit' }, { factor: 'vibrant_atmosphere', adjustment: 'increase', weight: 0.8, reasoning: 'Vibrant environment stimulates exploration' }, { factor: 'trendy_atmosphere', adjustment: 'increase', weight: 0.7, reasoning: 'Trendy venues offer new experiences' });
                break;
            default:
                adjustments.push({ factor: 'comfortable_atmosphere', adjustment: 'increase', weight: 0.8, reasoning: 'Comfortable setting suits neutral mood' }, { factor: 'casual_atmosphere', adjustment: 'increase', weight: 0.7, reasoning: 'Casual environment is versatile and welcoming' });
        }
        return adjustments;
    }
    /**
     ...
     */
    generatePriceAdjustments(request, type) {
        const adjustments = [];
        switch (type) {
            case 'celebratory':
                adjustments.push({
     ...
     */
    filterAndRankRecommendations(recommendations, request) {
        ...
        if (request.userPreferences?.dietaryRestrictions) {
            ...
            filtered = filtered.filter(rec => {
                return !request.userPreferences.dietaryRestrictions.some(restriction => restrictionMap[restriction]?.includes(rec.cuisineType));
            });
        }
     ...
     */
    generateRecommendationReasoning(request, type) {
        const { primaryEmotion, intensity } = request;
     ...
     */
    calculateRecommendationConfidence(request, type) {
     ...
     */
    initializeCelebratoryDiningMappings() {
     ...
     */
    initializeTherapeuticCuisineMappings() {
...
exports.MoodBasedRecommendationService = MoodBasedRecommendationService;
//# sourceMappingURL=moodBasedRecommendation.js.map