...
exports.EmotionCuisineMappingService = void 0;
class EmotionCuisineMappingService {
    constructor() {
     ...
     */
    getCuisineRecommendations(request) {
        ...
        if (request.userPreferences?.dietaryRestrictions) {
            recommendations = this.filterByDietaryRestrictions(recommendations, request.userPreferences.dietaryRestrictions);
        }
        // Boost recommendations that match user's preferred cuisines
        if (request.userPreferences?.cuisineTypes) {
            recommendations = this.boostPreferredCuisines(recommendations, request.userPreferences.cuisineTypes);
        }
     ...
     */
    filterByDietaryRestrictions(recommendations, restrictions) {
        ...
        return recommendations.filter(rec => {
            return !restrictions.some(restriction => restrictionMap[restriction]?.includes(rec.cuisineType));
        });
    }
    /**
     * Boosts match scores for user's preferred cuisines
     */
    boostPreferredCuisines(recommendations, preferredCuisines) {
     ...
     */
    adjustForIntensity(recommendations, emotion, intensity) {
        return recommendations.map(rec => {
            let adjustment = 0;
...
exports.EmotionCuisineMappingService = EmotionCuisineMappingService;
//# sourceMappingURL=emotionCuisineMapping.js.map